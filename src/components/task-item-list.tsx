...
const TaskListItems = () => {
    ...
    const sendMerlin = async () => {
        ...
        try {
            ...
            await TodoItemService.rearrange(arrangedItems)
        } catch (error) {
            console.log(error);
        } finally {
            setIsLoading(false)
    ...
    const removeSuggestion = async () => {
        try {
            setIsRemoving(true)
            await TodoItemService.removeArrange(todo_id)
        } catch (error) {
            console.log(error);
        } finally {
            setIsRemoving(false)
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div className="flex gap-4 items-center fill-current">
                    ...
                    <div className="flex gap-4">
                        ...
                        {items?.[0].number != null 
                            &&
                            <div className="hover:scale-95 duration-100 cursor-pointer" onClick={removeSuggestion}>
                                <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-5 ${isRemoving && 'animate-spin'}`}>
